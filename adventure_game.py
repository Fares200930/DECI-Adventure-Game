...
def cave(items):
    ...
    if "wand of radiance" in items or "obsidian blade" in items:
        ...
        if "cracked wand" in items:
            print_pause("You discard your cracked wand and take the Wand of Radiance with you.")
...
def house(items, health, total_score, current_creature):
    ...
    action = get_valid_input("Would you like to (1) fight or (2) run away?\n", ["1", "2"])
    if action == "1":
        # Display the backpack and allow the player to choose an item
        chosen_item = backpack(items)

        # Determine the player's tool power
        player_power = tools.get(chosen_item, 0)

        if player_power >= creatures[creature]["power"]:
            print_pause(f"As the {creature} moves to attack, you raise your {chosen_item}.")
            if "wand of radiance" in items:
                print_pause(f"The {chosen_item} shines brightly in your hand as you brace yourself.")
                print_pause(f"But the {creature} takes one look at your shiny new wand and runs away!")
            else:
                print_pause(f"You swing your {chosen_item} with all your might and defeat the {creature}!")
            ...
            response = get_valid_input("Would you like to play again? (Y/n)\n", ["y", "n"])
            if response == "y":
                print_pause("Restarting the game while saving your score...")
                play_game(total_score)  # Restart the game with the current score
            else:
                print_pause(f"Thanks for playing! Your final score was {total_score}. Goodbye!")
                exit()
        else:
            print_pause(f"You do your best, but your {chosen_item} is no match for the {creature}.")
            health["hp"] -= 20
            print_pause(f"The {creature} attacks you! Your health is now {health['hp']}.")
            if health["hp"] <= 0:
...
def forest(items):
    ...
    # Update items or state based on the event
    if "healing potion" not in items and "potion" in event:
        ...
        if "cracked wand" in items:
            print_pause("You discard your cracked wand and take the steel sword with you.")
            items.remove("cracked wand")
...
def backpack(items):
    ...
    for i, item in enumerate(items, start=1):
        ...
        print_pause(f"{i}. {item} (Power: {power})")
    
    while True:
        choice = get_valid_input("Choose an item by number:\n", [str(i) for i in range(1, len(items) + 1)])
        chosen_item = items[int(choice) - 1]
        print_pause(f"You have chosen the {chosen_item} (Power: {tools[chosen_item]}).")
        return chosen_item

def play_again(total_score):
    """Prompt the player to play again or exit."""
    response = get_valid_input("Would you like to play again? (yes/no)\n", ["yes", "no"])
    if response == "yes":
...
def play_game(total_score=0):
    ...
    print_pause("Rumor has it that a dangerous creature is somewhere around here, "
                "and has been terrifying the nearby village.")
    print_pause("In front of you is a house.")
    print_pause("To your right is a dark cave.")
    print_pause("In your hand you hold your trusty (but not very effective) cracked wand.")
    ...
    while True:
        ...
        choice = get_valid_input("(Please enter 1, 2, 3, or 4.)\n", ["1", "2", "3", "4"])
        if choice == "1":
            current_creature, total_score = house(items, health, total_score, current_creature)
        elif choice == "2":
            cave(items)
        elif choice == "3":
            forest(items)

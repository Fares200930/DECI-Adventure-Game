...
def play_game(total_score=0):
    ...
    max_turns = 20  # Maximum number of turns allowed
    turns = 0  # Track the number of turns
    ...
    # Main game loop with multiple ending conditions
    while total_score < 100 and health["hp"] > 0 and turns < max_turns:
        print_pause(f"Your current health: {health['hp']}")
        print_pause(f"Your current score: {total_score}")
        print_pause(f"Turns remaining: {max_turns - turns}")
    ...
    if total_score >= 100:
        print_pause(
            "Congratulations! You won by reaching the target score of "
            "100!"
        )
    elif health["hp"] <= 0:
        print_pause("Game over! You ran out of health.")
    elif turns >= max_turns:
        print_pause("Game over! You used all your turns.")
    else:
